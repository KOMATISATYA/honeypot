...
# -------------------------
# Background RL + Session Logic
# -------------------------
async def post_process_intelligence(persona, action, intel, persona_reply):

    print("\n🔥 Background RL Started")
    ...
    # Session Detection
    closing_keywords = ["thank you", "goodbye", "ok noted"]
    session_end = any(keyword in persona_reply.lower() for keyword in closing_keywords)
...
async def process_message(message, history, session_id, previous_intel):
    ...
    classification = json.loads(res.messages[-1].content)
...
"""
    ...
    intel = None
    try:
        intel = IntelligenceSchema.model_validate_json(
            extraction_result.messages[-1].content
        )
    except Exception as e:
        print("Extraction Parsing Failed:", str(e))
    ...
    # Background learning happens AFTER response
    if intel:
        asyncio.create_task(
            post_process_intelligence(persona, action, intel, persona_reply)
        )
