...
# -------------------------
# INTEL COMPLETION CHECK
# -------------------------
def is_intel_complete(intel: dict):
    if not intel:
        return False
    ...
    for field in required_fields:
        if not intel.get(field) or len(intel.get(field)) == 0:
            return False
...
async def honeypot(payload: dict, x_api_key: str = Header(None)):
    ...
    if scam:
        ...
        if intel:
            ...
            if is_intel_complete(cumulative_intel):

                print(f"\n🔥 INTEL COMPLETE → TRIGGERING CALLBACK for {session_id} 🔥")

                await send_callback(
                    session_id,
                    len(memory.get_history(session_id)),
                    cumulative_intel
                )
                ...
                print(f"✅ Session {session_id} completed via INTEL STOP.")
